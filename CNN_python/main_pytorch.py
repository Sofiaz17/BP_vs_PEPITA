    ...
    for epoch in range(train_epochs):  # loop over the dataset multiple times
        ...
        if eta_decay:
            if epoch in decay_epochs:
                if learn_type == 'BP':
                    ...
                    file.write('\nAt epoch {} learning rate decreased to {}'.format(epoch,eta))
                    file.close()
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if learn_type == 'BP':
                    ...
                    if 'fc' in layers_key[l] and 'bn' not in layers_key[l]:
                        ...
                        if l == first_fc-1 and first_fc == len(layers_act): # only fc layers: case with only one fc layer after conv layers
                            ...
                            if is_pool == False:
                                ...
                                # print(f"if delta_w: {delta_w.shape}")
                            else:
                                delta_w = -error.T @ net.pool(mod_layers_act[-2]).flatten(1)
                                # print(f"else delta_w: {delta_w.shape}")
                ...
                # for i, act in enumerate(v_w_all):        
                #     print(f"Dimension of v_w_all[{i}]: {act.shape}")
                
                # for i, act in enumerate(delta_w_all):        
                #     print(f"Dimension of delta_w_all[{i}]: {act.shape}")

                # #for i, act in enumerate(layers_key):        
                # print(f" layers_key[]: {layers_key}")
        ...
        filename = savepath+'/res_summary_'+exp_name+'.txt'
        file = open(filename,'a')
        file.write(f"\n[{epoch + 1}, {i + 1}] loss: {running_loss / i}")
